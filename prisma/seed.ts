...
async function main() {
  ...
  const order1 = await prisma.order.create({
    data: {
      ...
      clientId: client1.id,
      status: "in_progress",
      deliveryDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 días desde ahora
      marginPercent: 35,
        ...
        create: [
          {
            pieceId: piece1.id,
            quantity: 5,
            unitCost: 6.25, // 25g * 15€/kg = 0.375€ + overhead
            unitPrice: 9.99,
          },
          {
            pieceId: piece2.id,
  ...
  const itemsTotalCost = order1.items.reduce(
    (sum: number, item: any) => sum + item.unitCost * item.quantity,
    0
  )
  const itemsTotalPrice = order1.items.reduce(
  ...
  const order2 = await prisma.order.create({
    data: {
      ...
      clientId: client2.id,
      status: "pending",
      deliveryDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000), // 14 días
      marginPercent: 30,
  ...
  const items2TotalCost = order2.items.reduce(
    (sum: number, item: any) => sum + item.unitCost * item.quantity,
    0
  )
  const items2TotalPrice = order2.items.reduce(
    (sum: number, item: any) => sum + item.unitPrice * item.quantity,
    0
  )
  ...
  const maintenance1 = await prisma.maintenance.create({
    data: {
      ...
      notes: "Calibración manual, sem partes nuevas",
    },
  })
